...
- [release/claude/](release/claude) — Claude 측 다운로드 자산: `stata-mcp.dxt` (Desktop) + `cowork-skills/` (슬래시 명령 스킬 6종 — `/stata-setup`, `/stata-exec`, `/stata-pull`, `/stata-data-fullcontext`, `/stata-graph-get`, `/stata-instruction`). 등록 방법은 INSTALL.md 7장 참고
- **한 번에 받기**: [`claude.zip` (latest)](../../releases/tag/claude-latest) — `release/claude/` 전체를 zip 으로 (자동 빌드)

## 변경 이력

[Git 커밋 로그](../../commits/main) 참고. (이 repo 는 continuous-build 방식으로 운영 — `main` 푸시 시 `claude-latest` Release 가 자동 재빌드되며, versioned tag/release 는 사용 안 함)
