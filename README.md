# Stata MCP Java — Releases

Stata와 Claude(Desktop / Code)를 MCP(Model Context Protocol)로 연결하는 도구의 **공개 배포 저장소**입니다. 소스 코드는 비공개이며, 이 저장소는 빌드된 배포 파일과 사용자 문서만 제공합니다.

## 다운로드

[**Releases 페이지**](../../releases) 에서 최신 버전 자산을 받으세요.

| 파일 | 설명 |
|---|---|
| `stata-mcp.dxt` | **Claude Desktop 원클릭 설치 번들** — MCP 서버 jar + stdio 브릿지 내장 |
| `stata-drone.jar` | Stata 내부 실행 드론 (포트 8001) |
| `mcp_connect.ado` | Stata 드론 연결 명령어 |
| `llm.ado` | Stata push 명령어 (`llm push > cmd`) |
| `stata_mcp_instructions.md` | Claude 기본 지침 (간결, ~450 토큰) — `.dxt` 에 동일 사본 번들 |
| `stata_mcp_instructions_example_full.md` | Claude 지침 예시 (상세, ~1500 토큰) — 대안 |

> Cursor / Claude Code 사용자: `stata-mcp.dxt` 는 zip 컨테이너입니다. 압축 해제 시 `server/stata-mcp-server.jar` 추출 가능 — [INSTALL.md](INSTALL.md) 3-B 참고.

## 가이드

- [INSTALL.md](INSTALL.md) — 설치 가이드 (다운로드 → 설정 → MCP 등록 → 스킬 등록)
- [USAGE.md](USAGE.md) — 사용 가이드 (시작 순서, Claude Desktop / Claude Code 사용법, 문제 해결)
- [skill-bundles/](skill-bundles) — (선택) 코워크 슬래시 명령 스킬 6종 (`/stata-setup`, `/stata-exec`, `/stata-pull`, `/stata-data-fullcontext`, `/stata-graph-get`, `/stata-instruction`) — 등록 방법은 INSTALL.md 8장 참고

## 변경 이력

각 [Release 페이지](../../releases) 본문에 해당 버전 변경 내역이 포함되어 있습니다.

## 사전 요구 사항

| 항목 | 버전 |
|---|---|
| Java | 17 이상 — [Eclipse Temurin](https://adoptium.net/) |
| Node.js | 18 이상 — [nodejs.org](https://nodejs.org/) |
| Stata | 17 이상 (19 권장) |
| Claude Desktop 또는 Claude Code | 최신 버전 |

## 라이선스

Copyright (c) 2024 mhjung0822.
