...
- [release/claude/](release/claude) — Claude 측 다운로드 자산: `stata-mcp.dxt` (Desktop) + 슬래시 명령 스킬 7종 (`/stata-setup`, `/stata-exec`, `/stata-pull`, `/stata-data-fullcontext`, `/stata-graph-get`, `/stata-graph-export`, `/stata-instruction`). **Claude Code 는 `stata-skills.zip` (7종 묶음) 한 번에**, claude.ai 웹은 `cowork-skills/` 의 개별 zip 업로드. 등록 방법은 INSTALL.md 7장 참고
