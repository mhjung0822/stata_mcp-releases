...
[**Releases 페이지**](../../releases) 에서 최신 버전 자산을 받으세요.

| 파일 | 설명 |
|---|---|
| `stata-mcp-server.jar` | MCP 서버 (Spring Boot, Streamable HTTP, 포트 8080) — Cursor/Code 사용자 standalone 용 |
| `stata-mcp.dxt` | **Claude Desktop 설치 wrapper + 서버 jar 번들** — `mcp-remote` 로 stdio↔HTTP 자동 등록 |
| `stata-drone.jar` | Stata 내부 실행 드론 (포트 8001) |
| `mcp_connect.ado` | Stata 드론 연결 명령어 (`mcp_find_server` 로 jar 경로 잡고 spawn 가능) |
| `mcp_find_server.ado` | `.dxt` 안에 번들된 `stata-mcp-server.jar` 경로 탐지 (Claude Extensions dir 검색) |
...
- [skill-bundles/](skill-bundles) — (선택) 코워크 슬래시 명령 스킬 6종 (`/stata-setup`, `/stata-exec`, `/stata-pull`, `/stata-data-fullcontext`, `/stata-graph-get`, `/stata-instruction`) — 등록 방법은 INSTALL.md 8장 참고
