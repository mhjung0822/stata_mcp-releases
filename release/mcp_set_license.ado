...
program mcp_set_license
    ...
    * 다음 붙여넣은 줄을 키로 삼켜버린다.
    if `"`key'"' == "" {
        ...
        * 매크로 이름 주의: 언더스코어 시작 글로벌은 시스템 예약이라 global 명령으로
        * 조작 불가(invalid syntax), _request 이름은 8자 제한 — 그래서 mcpkey.
        display _request(mcpkey)
        local key = strtrim(`"$mcpkey"')
        capture macro drop mcpkey
        if `"`key'"' == "" {
