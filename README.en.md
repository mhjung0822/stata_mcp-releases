...
- **Skills** *(recommended)*: [`stata-skills-all-en.zip`](https://raw.githubusercontent.com/mhjung0822/stata_mcp-releases/main/claude-plugins/stata-skills-all-en.zip) → **unzip it**, then upload the per-skill zips inside via **Settings → Skills → Upload**. Upload once; it syncs to every device on the same account. Includes 9 slash commands (`/stata-exec`, `/stata-help`, …), an editable working-style instruction skill, and a panel-merge skill. (Korean edition: [`stata-skills-all.zip`](https://raw.githubusercontent.com/mhjung0822/stata_mcp-releases/main/claude-plugins/stata-skills-all.zip) — the skills share names, so install one language pack per account.)
